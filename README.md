...
```sh
docker run --rm -v $PWD:/tmp ghcr.io/degauss-org/nlcd:0.3.0 my_address_file_geocoded.csv
```

will produce `my_address_file_geocoded_nlcd_0.3.0_400m_buffer.csv` with the following added columns describing the [land classification](https://www.mrlc.gov/data/legends/national-land-cover-database-class-legend-and-description) within a 400 meter buffer radius of each point:
...
```sh
docker run --rm -v $PWD:/tmp ghcr.io/degauss-org/nlcd:0.3.0 my_address_file_geocoded.csv 800
```

will produce `my_address_file_geocoded_nlcd_0.3.0_800m_buffer.csv`, and all output will be values within an 800 m buffer. 
